...
The logistic model used is very similar with the one from Part4. \\The input layer is of size 128*128, and is fully connected to the output layer. We are still using one-hot encoding for the outputs: [0, 1], [1, 0] represent adjacent and unadjacent pairs respectively. \\ With experiments, we noticed the performance rate is a little higher with L1 regularization, so we chose to use Lasso regularization for our model. Tuning the parameters with grid search, We finalized $\lambda$= 0.0075  for the decay penalty and $\alpha$ = 0.005 for gradient descent. \\
...
\textbf{Results shown below: (ordered with similarity in decrease)}
